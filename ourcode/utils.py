...
#     )
#     return save_path
def to_feature_filename(output_dir, dataset, subset, model_name, pool=None, num_samples=None, prompt=None, batch_size=None, caption_idx=None, language=None):
    save_name = f"{model_name.replace('/', '_')}"
    ...
    if num_samples:
        save_name += f"num_samples-{num_samples}"
    save_path = os.path.join(
        output_dir, 
        dataset, 
        subset,
